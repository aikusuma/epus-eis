    ...
    <div className='*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs md:grid-cols-2 lg:grid-cols-4'>
      <Card className='@container/card'>
        <CardHeader>
          ...
          <CardTitle className='text-2xl font-semibold tabular-nums @[250px]/card:text-3xl'>
            {summaryData.totalKunjungan.toLocaleString('id-ID')}
          </CardTitle>
          <CardAction>
            <Badge variant='outline'>
              {summaryData.pertumbuhanPersen >= 0 ? (
                <IconTrendingUp />
              ) : (
                <IconTrendingDown />
              )}
              {summaryData.pertumbuhanPersen >= 0 ? '+' : ''}
              {summaryData.pertumbuhanPersen.toFixed(1)}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className='flex-col items-start gap-1.5 text-sm'>
          <div className='line-clamp-1 flex gap-2 font-medium'>
            {summaryData.pertumbuhanPersen >= 0 ? 'Naik' : 'Turun'} dari bulan
            lalu{' '}
            {summaryData.pertumbuhanPersen >= 0 ? (
      ...
      <Card className='@container/card'>
        <CardHeader>
          ...
          <CardTitle className='text-2xl font-semibold tabular-nums @[250px]/card:text-3xl'>
            {summaryData.kasusIspa.toLocaleString('id-ID')}
          </CardTitle>
          <CardAction>
            <Badge variant='outline'>
              {summaryData.kasusIspaTrend < 0 ? (
                <IconTrendingDown />
              ) : (
                <IconTrendingUp />
              )}
              {summaryData.kasusIspaTrend >= 0 ? '+' : ''}
              {summaryData.kasusIspaTrend.toFixed(1)}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className='flex-col items-start gap-1.5 text-sm'>
          <div className='line-clamp-1 flex gap-2 font-medium'>
            {summaryData.kasusIspaTrend < 0 ? 'Turun' : 'Naik'} dari bulan lalu{' '}
            {summaryData.kasusIspaTrend < 0 ? (
              <IconTrendingDown className='size-4' />
            ) : (
              <IconTrendingUp className='size-4' />
      ...
      <Card className='@container/card'>
        <CardHeader>
          ...
          <CardTitle className='text-2xl font-semibold tabular-nums @[250px]/card:text-3xl'>
            {summaryData.kasusHipertensi.toLocaleString('id-ID')}
          </CardTitle>
          <CardAction>
            <Badge variant='outline'>
              {summaryData.kasusHipertensiTrend > 0 ? (
                <IconTrendingUp />
              ) : (
                <IconTrendingDown />
              )}
              {summaryData.kasusHipertensiTrend >= 0 ? '+' : ''}
              {summaryData.kasusHipertensiTrend.toFixed(1)}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className='flex-col items-start gap-1.5 text-sm'>
